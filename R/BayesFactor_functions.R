...
repposteriorplot <- function(n.orig, r.orig, n.rep, r.rep, BFIN = "no", BF01 = 0, xlab=expression(Correlation ~ rho))
{
  ...
  plot ( function(rho) PosteriorRho(rho,(n.orig-1),r.orig), -1, 1 
         , xlim = c(mix,max), ylim = c(0,my-4.3), lwd=2, lty=3, col= 1,
         ylab="Density", xlab=" ", main = " ") 
  axis(1)
  axis(2)
  mtext(xlab, side=1, line = 2.8, cex=1.2)
  par ( new = TRUE)
  plot ( function(rho) ReplicationPosteriorRho(rho), -1, 1, 
         xlim = c(mix,max), ylim = c(0,my-4.3), lwd=2, lty=1, col= 1,
         ylab="Density", xlab=" ", main = " ") 
  ...
  legend(mix , my-4.225 , c("Prior","Posterior"), lwd= 2, lty=c(3,1) ,bty = 'n' , cex= 1.5)  
  ...
  if( BF10 >= 1){ 
    text(.5, my-4.75 , labels = substitute(paste(BF['01'], " = ", v), list(v=signif(BF01, digits=3))) , cex = 1)
    text(.5, my-5.5  , labels = substitute(paste(BF[10], " = ", v), list(v=signif(BF10, digits=3))) , cex = 1)
    
  }
  if( BF10 < 1 & BF10 > .000501){ 
    text(.5, my-4.75 , labels = substitute(paste(BF['01'], " = ", v), list(v=round(BF01,digits=2))) , cex = 1)
    text(.5, my-5.5 , labels = substitute(paste(BF[10], " = ", v), list(v=round(BF10,digits=2))) , cex = 1)
